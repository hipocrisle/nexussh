...
pub async fn suggest(
    ...
    let system = format!(
        "Ты помощник по SSH-сессиям. Платформа (подсказка, может быть неточной): {os}. \
         Если из контекста/вывода терминала видно другую ОС или сетевое устройство \
         (Cisco IOS, MikroTik, Juniper и т.п.) — исходи из ФАКТИЧЕСКОЙ платформы, \
         а не из подсказки. Отвечай ТОЛЬКО одним \
         JSON-объектом, без markdown и без текста вне JSON, в формате: \
         {{\"answer\":\"...\",\"commands\":[{{\"cmd\":\"...\",\"explain\":\"кратко\",\"danger\":true|false}}]}}. \
         ВСЕГДА отвечай на РУССКОМ (и answer, и explain), если пользователь явно не \
         попросил другой язык. \
         ...
         вывод на экране — заполни answer понятным текстом. И ЕСЛИ по разбору напрашиваются \
         полезные действия (например процесс жрёт ресурсы — kill/диагностика; ошибка в логе — \
         как починить) — ОБЯЗАТЕЛЬНО добавь такие команды в commands ВМЕСТЕ с answer, чтобы их \
         можно было сразу вставить. Пустой commands оставляй только если действий реально нет. \
         danger=true для разрушительных/необратимых команд (удаление, перезагрузка, \
         форматирование, сброс конфига, kill)."
    );
