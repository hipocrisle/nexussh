...
**AI-ассистент команд** *(новое)*
- Опиши задачу простым языком — получи команды для вставки (никогда не запускаются сами) или текстовое объяснение вывода на экране
- Понимает платформу: определяет цель (Linux, Cisco IOS, MikroTik, Juniper…) по приглашению и подстраивает синтаксис
- Опциональный режим «AI видит экран» отправляет последние ~40 строк для точности — пароли и ключи вырезаются на стороне клиента; по умолчанию выключен, иначе наружу уходит только ярлык платформы
- Доступ по одобрению, с дневными лимитами на пользователя; подсказки копируются и вставляются только по твоему клику

**Command palette** *(новое, десктоп)*
- Один хоткей (по умолчанию `Ctrl+Shift+Z`, настраивается) — fuzzy-поиск по хостам, сниппетам, открытым вкладкам, действиям и настройкам
- Только с клавиатуры: стрелки, Enter, `Tab` — переход по секциям, префиксы `@ # > /` для фильтра; плюс строка «спросить AI»
...
- Сниппеты (сетка команд, хоткеи, синк)
